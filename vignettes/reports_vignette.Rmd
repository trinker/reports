...
Install `r HR2("http://www.python.org/", text="Python")` and make it accessible from the command line `r yt("http://youtu.be/sg_lKU3n-co")`

The user may then click on the bash/batch file generated by `r FUN("local_host")` and run the presetation locally. `r yt("http://youtu.be/VlqpiKHz7Gw")` (note that this file must be clicked on outside of RStudio).  This automatically creates a local host and opens the directory of the presentation for automatic access to locally hosted presentations.
